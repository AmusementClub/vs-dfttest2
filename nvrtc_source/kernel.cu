// this file is derived from fftw 3.10
...
template <int n>
__device__
static void rdft(float data[/* (n / 2) * stride * 2 + 2 */], int stride = 1);

// ./gen_r2cf.native -standalone -with-rs 2 -with-csr 2 -with-csi 2 -fma -n 16
template <>
__device__
void rdft<16>(float data[/* 16 * stride + 2 */], int stride) {
    using E = float;
    ...
    Ci[8 * stride] = T35 - T34;
    {
        ...
        T15 = T3 - T6;
        T30 = T22 + T29;
        Cr[12 * stride] = FNMS(KP707106781, T30, T15);
        Cr[4 * stride] = FMA(KP707106781, T30, T15);
        T31 = T13 - T10;
        T32 = T29 - T22;
        Ci[4 * stride] = FMA(KP707106781, T32, T31);
        Ci[12 * stride] = FMS(KP707106781, T32, T31);
    }
    {
        E T33;
        E T36;
        ...
        T33 = T7 + T14;
        T36 = T34 + T35;
        Cr[16 * stride] = T33 - T36;
        Cr[0 * stride] = T33 + T36;
        T41 = FMA(KP707106781, T40, T37);
        T48 = T44 + T47;
        Cr[14 * stride] = FNMS(KP923879532, T48, T41);
        Cr[2 * stride] = FMA(KP923879532, T48, T41);
    }
    T53 = FNMS(KP707106781, T50, T49);
    T56 = T54 - T55;
    Ci[2 * stride] = FMS(KP923879532, T56, T53);
    Ci[14 * stride] = FMA(KP923879532, T56, T53);
    {
        E T57;
        E T58;
        E T51;
        ...
        T58 = T55 + T54;
        Cr[10 * stride] = FNMS(KP923879532, T58, T57);
        Cr[6 * stride] = FMA(KP923879532, T58, T57);
        T51 = FMA(KP707106781, T50, T49);
        T52 = T47 - T44;
        Ci[6 * stride] = FMA(KP923879532, T52, T51);
        Ci[10 * stride] = FMS(KP923879532, T52, T51);
    }
    Ci[0 * stride] = 0.0f;
    Ci[16 * stride] = 0.0f;
}
...
template <int n>
__device__
static void irdft(float data[/* (n / 2) * stride * 2 + 2 */], int stride = 1);

// ./gen_r2cb.native -standalone -with-rs 2 -with-csr 2 -with-csi 2 -fma -n 16
template <>
__device__
void irdft<16>(float data[/* 16 * stride + 2 */], int stride) {
    using E = float;
    ...
    }
    {
        ...
        T16 = T12 + T15;
        R0[8 * stride] = FNMS(KP2_000000000, T16, T9);
        R0[0 * stride] = FMA(KP2_000000000, T16, T9);
        T55 = FNMS(KP2_000000000, T48, T47);
        T56 = T53 - T50;
        R0[14 * stride] = FNMS(KP1_414213562, T56, T55);
        R0[6 * stride] = FMA(KP1_414213562, T56, T55);
    }
    {
        E T57;
        E T58;
        ...
        T58 = T52 + T51;
        R0[12 * stride] = FNMS(KP2_000000000, T58, T57);
        R0[4 * stride] = FMA(KP2_000000000, T58, T57);
        T25 = FMA(KP1_414213562, T24, T19);
        T36 = FMA(KP414213562, T35, T30);
        R1[8 * stride] = FNMS(KP1_847759065, T36, T25);
        R1[0 * stride] = FMA(KP1_847759065, T36, T25);
    }
    {
        E T37;
        E T38;
        ...
        R1[12 * stride] = FNMS(KP1_847759065, T38, T37);
        R1[4 * stride] = FMA(KP1_847759065, T38, T37);
        T45 = FMA(KP1_414213562, T40, T39);
        T46 = FMA(KP414213562, T42, T43);
        R1[10 * stride] = FNMS(KP1_847759065, T46, T45);
        R1[2 * stride] = FMA(KP1_847759065, T46, T45);
    }
    {
        E T49;
        E T54;
        ...
        T54 = T50 + T53;
        R0[10 * stride] = FNMS(KP1_414213562, T54, T49);
        R0[2 * stride] = FMA(KP1_414213562, T54, T49);
        T41 = FNMS(KP1_414213562, T40, T39);
        T44 = FNMS(KP414213562, T43, T42);
        R1[6 * stride] = FNMS(KP1_847759065, T44, T41);
        R1[14 * stride] = FMA(KP1_847759065, T44, T41);
    }
}

template <int block_size, int radius=0>
__launch_bounds__(32)
__global__ void rfft_kernel(cuFloatComplex * __restrict__ dst, const float * __restrict__ src, int num_blocks) {
    constexpr int warp_size = 32;
    constexpr int warps_per_block = 1;

    constexpr int transpose_stride = (warp_size % block_size == 0) ? block_size + 1 : block_size;
    __shared__ cuFloatComplex shared_transpose_buffer[warps_per_block * block_size * transpose_stride];
    ...
    if (lane_id < block_size) {
        ...
        for (int block_id = blockIdx.x * warps_per_block + threadIdx.x / warp_size; block_id < num_blocks; block_id += gridDim.x * warps_per_block) {
            cuFloatComplex thread_data[(2 * radius + 1) * block_size];

            #pragma unroll
            for (int i = 0; i < 2 * radius + 1; i++) {
                auto local_src = &src[(block_id * (2 * radius + 1) + i) * block_size * block_size];
                ...
                for (int j = 0; j < block_size; j++) {
                    ((float *) transpose_buffer)[j * transpose_stride + lane_id] = local_src[j * block_size + lane_id];
                }
                ...
                __syncwarp(active_mask);
                rdft<16>((float *) local_thread_data);
                ...
                if (lane_id < block_size / 2 + 1) {
                    ...
                    dft<16>((float *) local_thread_data);
                }
            }
            ...
            if (lane_id < block_size / 2 + 1) {
                ...
                auto local_dst = &dst[(block_id * (2 * radius + 1) * block_size) * (block_size / 2 + 1)];
                #pragma unroll
                for (int i = 0; i < (2 * radius + 1) * block_size; i++) {
                    local_dst[i * (block_size / 2 + 1) + lane_id].x = thread_data[i].x;
                    local_dst[i * (block_size / 2 + 1) + lane_id].y = thread_data[i].y;
                }
            }
        }
    }
...
template <int block_size, int radius=0>
__launch_bounds__(32)
static __global__ void irfft_kernel(float * __restrict__ dst, const cuFloatComplex * __restrict__ src, int num_blocks) {
    constexpr int warp_size = 32;
    constexpr int warps_per_block = 1;
    ...
    if (lane_id < block_size) {
        ...
        for (int block_id = blockIdx.x * warps_per_block + threadIdx.x / warp_size; block_id < num_blocks; block_id += gridDim.x * warps_per_block) {
            ...
            if (lane_id < block_size / 2 + 1) {
                #pragma unroll
                for (int i = 0; i < 2 * radius + 1; i++) {
                    auto local_src = &src[(block_id * (2 * radius + 1) + i) * block_size * (block_size / 2 + 1)];
                    auto local_thread_data = &thread_data[i * block_size];
                    #pragma unroll
                    for (int j = 0; j < block_size; j++) {
                        local_thread_data[j].x = local_src[j * (block_size / 2 + 1) + lane_id].x;
                        local_thread_data[j].y = local_src[j * (block_size / 2 + 1) + lane_id].y;
                    }
                }
            ...
            for (int i = 0; i < 2 * radius + 1; i++) {
                ...
                __syncwarp(active_mask);
                auto local_dst = &dst[(block_id * (2 * radius + 1) + i) * block_size * block_size];
                #pragma unroll
                for (int j = 0; j < block_size; j++) {
...
int get_rfft_grid_size(int radius, int block_size, int num_sms) {
    decltype(rfft_kernel<16>) * kernel;
    switch (block_size) {
        case 16:
            switch (radius) {
                case 0:
                    kernel = &rfft_kernel<16, 0>;
                    break;
                case 1:
                    kernel = &rfft_kernel<16, 1>;
                    break;
                case 2:
                    kernel = &rfft_kernel<16, 2>;
                    break;
                case 3:
                    kernel = &rfft_kernel<16, 3>;
                    break;
                default:
                    kernel = nullptr;
            }
    ...
    int num_blocks;
    if (auto result = cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks, kernel, 32, 0); result == cudaSuccess) {
        return num_blocks * num_sms;
    } else {
        fprintf(stderr, "rfft<%d, %d>: %s\n", block_size, radius, cudaGetErrorString(result));
        return 0;
    }
}
...
bool rfft(
    ...
    switch (block_size) {
        case 16:
            switch (radius) {
                case 0:
                    rfft_kernel<16, 0><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
                case 1:
                    rfft_kernel<16, 1><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
                case 2:
                    rfft_kernel<16, 2><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
                case 3:
                    rfft_kernel<16, 3><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
            }
        default:
            return false;
...
int get_irfft_grid_size(int radius, int block_size, int num_sms) {
    decltype(irfft_kernel<16, 0>) * kernel;
    switch (block_size) {
        case 16:
            switch (radius) {
                case 0:
                    kernel = &irfft_kernel<16, 0>;
                    break;
                case 1:
                    kernel = &irfft_kernel<16, 1>;
                    break;
                case 2:
                    kernel = &irfft_kernel<16, 2>;
                    break;
                case 3:
                    kernel = &irfft_kernel<16, 3>;
                    break;
                default:
                    kernel = nullptr;
            }
...
bool irfft(
    ...
    switch (block_size) {
        case 16:
            switch (radius) {
                case 0:
                    irfft_kernel<16, 0><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
                case 1:
                    irfft_kernel<16, 1><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
                case 2:
                    irfft_kernel<16, 2><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
                case 3:
                    irfft_kernel<16, 3><<<grid_size, 32, 0, stream>>>(dst, src, num_blocks);
                    return true;
            }
        default:
            return false;

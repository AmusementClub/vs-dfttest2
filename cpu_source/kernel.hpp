...
#define DK(name, value) const E name = E(value)
#define FMA(a, b, c) mul_add(a, b, c)
#define FMS(a, b, c) mul_sub(a, b, c)
...
static inline void transpose_8x8(Vec8f block[/* 8 */]) {
    for (int i = 0; i < 4; ++i) {
        ...
        block[i * 2 * stride] = temp1;
        block[(i * 2 + 1) * stride] = temp2;
    }
...
static inline void frequency_filtering(
    ...
    if (filter_type == 0) {
        for (int i = 0; i < (2 * radius + 1) * 16; i++) {
            ...
            auto multiplier = max((psd - sigma[i]) / (psd + 1e-15f), 0.0f);
    ...
    if (filter_type == 1) {
        for (int i = 0; i < (2 * radius + 1) * 16; i++) {
            ...
            block[i * 2] = select(mask, 0.0f, block[i * 2]);
            block[i * 2 + 1] = select(mask, 0.0f, block[i * 2 + 1]);
        }
    }
    if (filter_type == 2) {
        for (int i = 0; i < (2 * radius + 1) * 16; i++) {
    ...
    if (filter_type == 3) {
        for (int i = 0; i < (2 * radius + 1) * 16; i++) {
            auto psd = square(block[i * 2]) + square(block[i * 2 + 1]);
            auto multiplier = select(psd >= pmin && psd <= pmax, sigma[i], sigma2);
            block[i * 2] *= multiplier;
            block[i * 2 + 1] *= multiplier;
        }
    }
    ...
    if (filter_type == 5) {
        for (int i = 0; i < (2 * radius + 1) * 16; i++) {
            auto psd = square(block[i * 2]) + square(block[i * 2 + 1]);
            auto multiplier = pow(max((psd - sigma[i]) / (psd + 1e-15f), 0.0f), pmin);
            block[i * 2] *= multiplier;
            block[i * 2 + 1] *= multiplier;
        }
    }
    if (filter_type == 6) {
        for (int i = 0; i < (2 * radius + 1) * 16; i++) {
            auto psd = square(block[i * 2]) + square(block[i * 2 + 1]);
            auto multiplier = sqrt(max((psd - sigma[i]) / (psd + 1e-15f), 0.0f));
            block[i * 2] *= multiplier;
            block[i * 2 + 1] *= multiplier;
        }
    }
